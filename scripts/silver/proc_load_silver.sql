...
BEGIN
	...
	BEGIN TRY 
		...
		FROM (
			SELECT 
			*,
			ROW_NUMBER() OVER (PARTITION BY cst_id ORDER BY cst_create_date) flag_fresh
			FROM bronze.crm_cust_info 
			WHERE cst_id IS NOT NULL
			)t 
		WHERE flag_fresh = 1 ; -- select the most recent record per customer
		...
		SELECT  
			prd_id,
			REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, -- matching (id) column in the category table
			SUBSTRING(prd_key, 7, LEN(prd_key)) AS prd_key,  -- matching (sls_prd_key) column in sales details table
			prd_nm,
		...
		SELECT 
			...
			END sls_order_dt,   -- Will do the same thing for the other 2 Date Columns just in case (Future)
			CASE 
			...
			CASE 
				...
				ELSE sls_sales 
			END sls_sales,   -- Recalculate Sales if any original value is missing or incorrect
			sls_quantity, 
			CASE 
